...
class SessionManager
{
        ...
        public function __construct(RakLibServer $server, UDPServerSocket $socket)
        {
                ...
                $this->antiDDoS = new AntiDDoSGuard($this->server->getLogger());
        ...
        private function receivePacket()
        {
                ...
                if ($buffer !== null) {
                        ...
                        if ($this->antiDDoS !== null && $len > 0) {
                                $pid = ord($buffer[0]);
                                if (!$this->antiDDoS->checkIncomingPacket($source, $pid, $len)) {
                        ...
                        if ($id === RakLib::PACKET_ENCAPSULATED) {
                                ...
                                switch ($name) {
                                        ...
                                        case "antiddos":
                                                ...
                                                $cfg = @unserialize($value);
                                                if (is_array($cfg) && $this->antiDDoS !== null) {
                                                        $this->antiDDoS->setConfig($cfg);
                                                }
                                                break;
                                }
