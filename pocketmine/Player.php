...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
        ...
        protected function processLogin(){
                if(!$this->server->isWhitelisted(strtolower($this->getName()))){
                        $this->close($this->getLeaveMessage(), "Servidor en lista blanca.");
                ...
                $this->playedBefore = (((int) $nbt["lastPlayed"]) - ((int) $nbt["firstPlayed"])) > 1;
                ...
                $this->gamemode = ((int) $nbt["playerGameType"]) & 0x03;
        ...
        public function handleDataPacket(DataPacket $packet){
                ...
                if($packet::NETWORK_ID !== ProtocolInfo::LOGIN_PACKET){
                        $anti = $this->server->getAntiDDoS();
                        if($anti !== null){
                                if(!$anti->checkPlayerPacket($this, $packet::NETWORK_ID)){
                                        return; 
                                }
                        }
                }
                ...
                switch($packet::NETWORK_ID){
                        ...
                        case ProtocolInfo::LOGIN_PACKET:
                                ...
                                $anti = $this->server->getAntiDDoS();
                                if($anti !== null){
                                        if(!$anti->onLoginAttempt($this->getAddress())){
                                                $this->close("", "disconnectionScreen.invalidSession");
                                                break;
                                        }
                                        $bot = $anti->getAntiBot();
                                        if(!$bot->checkUsername($this->username)){
                                ...
                                if($packet->type === TextPacket::TYPE_CHAT){
                                        ...
                                        foreach(explode("\n", $packet->message) as $message){
                                                if(trim($message) != "" and strlen($message) <= 255 and $this->messageCounter-- > 0){
                                                        ...
                                                        if(mb_strlen($ev->getMessage(), "UTF-8") > 180){
                                                                $ev->setCancelled();
                                                        }
                                                        $this->server->getPluginManager()->callEvent($ev);
