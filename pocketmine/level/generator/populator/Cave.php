                        ...
                        for($z = $this->start->getFloorZ(); $z < $this->end->getFloorZ(); $z++){
                                ...
                                for($y = $this->end->getFloorY() - 1; $y >= $this->start->getFloorY(); $y--){
                                        ...
                                        if($yOffset > -0.7 and ($xOffset * $xOffset + $yOffset * $yOffset + $zOffset * $zOffset) < 1){
                                                ...
                                                // BUGFIX: the cave carver used to ONLY carve STONE/DIRT/GRASS.
                                                // That left SAND, GRAVEL, SANDSTONE and CLAY_BLOCK (placed by
                                                // GroundCover on beaches/deserts, and by RiverBankOverlay on
                                                // river banks) sitting on top of a column whose DIRT/STONE
                                                // had just been carved out -> floating "sand platforms" all
                                                // over the world, especially visible where rivers meet
                                                // mountains. We now carve every natural surface / near-
                                                // surface block so a cave opening is a clean hole rather
                                                // than a floating cap of unsupported sand.
                                                if($blockId == Block::STONE
                                                        or $blockId == Block::DIRT
                                                        or $blockId == Block::GRASS
                                                        or $blockId == Block::SAND
                                                        or $blockId == Block::GRAVEL
                                                        or $blockId == Block::SANDSTONE
                                                        or $blockId == Block::CLAY_BLOCK
                                                        or $blockId == Block::RED_SANDSTONE
                                                        or $blockId == Block::HARDENED_CLAY
                                                        or $blockId == Block::STAINED_CLAY){
                                                        if($y < 10){
                                                                $this->level->setBlockIdAt($xx, $y, $zz, Block::STILL_LAVA);
                                                        }else{
                                                                // When grass is broken by a cave, expose the dirt
                                                                // below as the new grass surface so the cave
                                                                // opening doesn't leave a weird dirt lip.
