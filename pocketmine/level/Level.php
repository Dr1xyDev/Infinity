        ...
        */
        ...
        public function generateChunkCallback(int $x, int $z, FullChunk $chunk){
                ...
                if(isset($this->chunkPopulationQueue[$index = Level::chunkHash($x, $z)])){
                        ...
                        $chunk->setProvider($this->provider);
                        // BUGFIX: force hasChanged=true so the provider actually persists
                        // the chunk. Without this, neighbor chunks that were generated
                        // inside a PopulationTask and then sent back here could be
                        // silently discarded if their hasChanged flag was not set
                        // (e.g. for Flat/Void generators, or when populateChunk only
                        // wrote blocks via setBlockIdAt on a chunk that was already
                        // marked as unchanged). This is the second half of the
                        // "trees cut vertically at chunk borders" fix.
                        $chunk->setChanged(true);
                        $this->setChunk($x, $z, $chunk, false);
                }else{
                        $chunk->setProvider($this->provider);
        ...
        public function generateChunk(int $x, int $z, bool $force = false){
                ...
                if(!isset($this->chunkGenerationQueue[$index = Level::chunkHash($x, $z)])){
                        ...
                        // BUGFIX: previously this dispatched a GenerationTask, which only
                        // runs generateChunk() and never populateChunk(). That left
                        // chunks with terrain but no trees / ores / flowers, and any
                        // tree that a NEIGHBOR's populateChunk() later tried to write
                        // across the border would land on an un-populated, un-locked
                        // chunk — producing the classic "tree cut vertically at chunk
                        // boundary" artefact. Using PopulationTask here (which both
                        // generates AND populates, and properly persists cross-border
                        // neighbor writes) matches the LiteCore behaviour recommended
        ...
        public function unloadChunks(bool $force = false){
                // BUGFIX (PHP 8.0+ TypeError: count(): Argument #1 ($value) must be
                // of type Countable|array, null given):
                //
                // On server restart with an existing world, doTick() runs BEFORE
                // any chunk is registered for unloading, so $this->unloadQueue may
                // still be null (the property used to be declared without a default
                // value). In PHP 7 count(null) silently returned 0; in PHP 8.0+
                // it throws a TypeError that crashes every level tick and produces
                // the "Could not tick level" spam. The declaration was also fixed
                // to `= []`, but this defensive guard catches any third-party
                // plugin that nulls the property at runtime.
                if(!is_array($this->unloadQueue)){
                        $this->unloadQueue = [];
                }
