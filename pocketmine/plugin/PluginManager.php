	...
	public function registerEvents(Listener $listener, Plugin $plugin){
		...
		foreach($reflection->getMethods(\ReflectionMethod::IS_PUBLIC) as $method){
			if(!$method->isStatic()){
				...
				// PHP 8.0: ReflectionParameter::getClass() is deprecated, use getType() instead.
				$paramClass = null;
				...
				}elseif(($refClass = $parameters[0]->getClass()) instanceof \ReflectionClass){
					$paramClass = $refClass->getName();
				}
				if($paramClass !== null and is_subclass_of($paramClass, Event::class)){
					$class = $paramClass;
