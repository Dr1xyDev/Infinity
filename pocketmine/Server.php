...
§9░▀▀▀░▀░▀░▀░░░▀▀▀░▀░▀░▀▀▀░░▀░░░▀░ §ev1.1
             ...
             §fInfinity by §b@Dr1xyDev

§7> §fPHP: §e' . PHP_VERSION . '
§7> §fAPI: §e' . $this->getApiVersion() . '
§7> §fProtocol: §e' . \pocketmine\network\protocol\Info::CURRENT_PROTOCOL . '
        ';
        ...
        public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
                ...
                try{
                        ...
                        $this->logger->info("Cargando propiedades y configuraciones...");
                        ...
                        if(file_exists($this->filePath . "src/pocketmine/resources/infinity_$lang.yml")){
                                $content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/infinity_$lang.yml");
                        }else{
                                $content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/infinity_eng.yml");
                        }

                        if(!file_exists($this->dataPath . "infinity.yml")){
                                @file_put_contents($this->dataPath . "infinity.yml", $content);
                        }
                        $internelConfig = new Config($file, Config::YAML, []);
                        $this->advancedConfig = new Config($this->dataPath . "infinity.yml", Config::YAML, []);
                        ...
                        if($this->pluginFolderLoad){
                                $this->pluginManager->registerInterface(FolderPluginLoader::class);
                                $this->logger->info("§9Folder Plugin loader §eON");
                        }else{
                                $this->logger->info("§9Folder plugin loader §cOFF");
                        }
                        ...
                        $this->antiDDoS = new AntiDDoS($this);
                        $this->pushAntiDDoSConfigToRakLib();
                        $this->getLogger()->info("§9> §aSistema anti-DDoS / anti-bot activo");
                        ...
                        if($cfgVer > $advVer){
                                $this->logger->notice("Your infinity.yml needs update");
        ...
        public function dispatchCommand(CommandSender $sender, $commandLine){
                ...
                $UnkCommand = $this->UnkCommand = $this->getAdvancedProperty("server.no-command", "§fComando Desconocido.");
                $sender->sendMessage($UnkCommand);
        ...
        public function pushAntiDDoSConfigToRakLib(): void{
                if($this->antiDDoS === null) return;
                $cfg = $this->antiDDoS->getRakLibConfig();
                $payload = serialize($cfg);
                foreach($this->network->getInterfaces() as $interface){
                        if(method_exists($interface, "sendAntiDDoSConfig")){
                                $interface->sendAntiDDoSConfig($payload);
                        }
                }
        }
